...
import (
	...
	coll "github.com/fcfcqloow/go-advance/collection"
...
func (self *String) AddAllHead(arg ...string) *String {
	*self = String(strings.Join(coll.CreateStringStream(arg...).Reverse().Add(self.AsPrimitive()).ToList(), ""))
	return self
}
func (self *String) AddAll(arg ...string) *String {
	*self = String(strings.Join(coll.CreateStringStream(self.AsPrimitive()).AddAll(arg...).ToList(), ""))
	return self
}
...
func (self *String) Split(str string) *coll.StringStream {
	return coll.CreateStringStream(strings.Split(self.AsPrimitive(), str)...)
}
...
func (self *String) SliceMatched(pattern string) *coll.StringStream {
	return coll.CreateStringStream(regexp.MustCompile(pattern).FindAllString(self.AsPrimitive(), -1)...)
}
